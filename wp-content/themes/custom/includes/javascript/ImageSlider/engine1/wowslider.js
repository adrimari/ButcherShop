...
{ 	
	...
	L("<div>").css({
		...
		"line-height":0}).append(
			a.find("li:first img:first").clone().css({width:"100%"}))
		.prependTo(a);
	...
	{
		N=L('<div>C<a href="#" style="display:none;position:absolute;left:0;top:0;width:100%;height:100%"></a></div>').css({
			position:"absolute",
			left:0,
			top:0,
			width:"100%",
		...
		if(f.addEventListener)
		{
			...
			f.addEventListener("touchstart",
				function(ar){
					...
					{
						al=ar.touches[0].pageX;ak=ar.touches[0].pageY;
						an=1;if(c){c(ar)}
					}
					else{an=0}
				},false);
			f.addEventListener("touchend",function(ar){an=0;if(aq){aq(ar)}if(!am&&ap){ap(ar)}},false)
		}
	}
...
// 
// 
// is wowslider.com using function
// replaced with escape char tab. and is gone now
// 
	var aj=a,d="\t";
	if(!d){return}
		d=ag(d);
