...
function wpbe_template_replacement( $template ) {
	global $wpdb;
	//$transaction_id = "SELECT `id` FROM `wp_wpsc_purchase_logs` ORDER BY `id` DESC LIMIT 1";
	//$qry_result = mysql_query($query) or die(mysql_error());
	//$row = mysql_fetch_array($qry_result);
	//$transaction_id = $row['id'];
	$transaction_id = $wpdb->get_var("SELECT `id` FROM `wp_wpsc_purchase_logs` ORDER BY `id` DESC LIMIT 1");
	$to_replace = array(
		...
		'transaction_id' => "Purchase no: ".$transaction_id
	);
