...
/***************************************************/
  ...
  function edit_item($key, $parameters) {
    if(isset($this->cart_items[$key])) {
			...
			$quantity = abs($parameters['quantity']);// - abs($this->cart_items[$key]->quantity);
...
/*			}
	...
	if(count($variation_names) > 0) {
		...
		$product_variation_str .= $this->custom_weight." Kg";
		$this->weight = $this->custom_weight;
	}
	...
	if (!$this->custom_units || ($this->units_grams == 0 && $this->units_kilograms == 0 && $this->units_ounces == 0 && $this->units_pounds == 0))
		 (float)$this->total_price = str_replace(",", "", number_format($this->unit_price * abs($this->quantity),2));//$this->total_price / $this->old_quantity * $this->quantity;
	else if (!$this->custom_units && $this->units_grams == '' && $this->units_kilograms == '' && $this->units_ounces == '' && $this->units_pounds == '')
		$this->total_price = 999;
	else 
		(float)$this->total_price = str_replace(",", "", number_format($this->unit_price * abs($this->quantity),2));//$this->unit_price;
	...
	*/
	function save_to_db($purchase_log_id) {
		...
		if ($this->custom_units) {
			$wpdb->query($wpdb->prepare("INSERT INTO `".WPSC_TABLE_CART_CONTENTS."` (`prodid`, `name`, `purchaseid`, `price`, `pnp`,`tax_charged`, `gst`, `quantity`, `donation`, `no_shipping`, `custom_message`, `files`, `meta`) VALUES ('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%d', '0', '%s', '%s', NULL)", $this->product_id, $this->product_name." (".$this->product_variation_str.")", $purchase_log_id, $this->unit_price, (float)$shipping, (float)$tax, (float)$tax_rate, $this->quantity, $this->is_donation, $this->custom_message, serialize($this->custom_file)));
		} else {
			$wpdb->query($wpdb->prepare("INSERT INTO `".WPSC_TABLE_CART_CONTENTS."` (`prodid`, `name`, `purchaseid`, `price`, `pnp`,`tax_charged`, `gst`, `quantity`, `donation`, `no_shipping`, `custom_message`, `files`, `meta`) VALUES ('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%d', '0', '%s', '%s', NULL)", $this->product_id, $this->product_name." (".$this->product_variation_str.")", $purchase_log_id, $this->unit_price, (float)$shipping, (float)$tax, (float)$tax_rate, $this->quantity, $this->is_donation, $this->custom_message, serialize($this->custom_file)));
		}
