...
function wpsc_pagination( $show = -1 ) {
	...
	while ( wpsc_have_pages() ) : wpsc_the_page();
		if ( wpsc_page_number() >= $start && wpsc_page_number() <= $end ) {
			//$page_url = str_replace('localhost/butchershop', 'shop.thebutchershop.co.za', wpsc_page_url());
			//$page_url = wpsc_product_search_url( $page_url );
			$page_url = home_url('/')."?page_id=10&category=".$_GET['category'];
			if ( !wpsc_page_is_selected() ) :
				$output .= '<div id="pages"><a href="' . $page_url."&page_number=".wpsc_page_number() . '" class="selected" style="font-size:13px">' . wpsc_page_number() . '</a></div>';
			else :
				$output .= '<span id="pages">'.wpsc_page_number().'</span>';//'<a href="' . $page_url . '" id="pages">' . wpsc_page_number() . '</a> ';
			endif;
		}
	endwhile;
...
function wpsc_next_products_link( $text = 'Next', $show_disabled = false ) {
	...
	//$page_url = str_replace('localhost/butchershop', 'shop.thebutchershop.co.za', $page_url);
	
	if ( $page_url ) {
		return '<div id="next"><a href="' . $page_url . '" style="font-size:13px">' . $text . '</a></div>';
	}
	
	if ( $show_disabled ) {
		return '<span id="next">' . $text . '</span>';
	}
...
function wpsc_previous_products_link( $text = 'Previous', $show_disabled = false ) {
	...
	if ( $page_url ) {
		return '<div id="previous"><a href="' . $page_url . '" style="font-size:13px">' . $text . '</a></div>';
	}
	
	if ( $show_disabled ) {
		return '<span id="previous">' . $text . '</span>';
	}
...
function wpsc_first_products_link( $text = 'First', $show_disabled = false ) {
	...
	if ( $page_url && wpsc_current_page() > 1 ) {
		return '<div id="first"><a href="' . $page_url . '" style="font-size:13px">' . $text . '</a></div>';
	}
	
	if ( $show_disabled ) {
		return '<span id="first">' . $text . '</span>';
	}
...
function wpsc_last_products_link( $text = 'Last', $show_disabled = false ) {
	...
	if ( $page_url && wpsc_current_page() < $wpsc_query->page_count ) {
		return '<div id="last"><a href="' . $page_url . '" style="font-size:13px">' . $text . '</a></div>';
	}
	
	if ( $show_disabled ) {
		return '<span id="last">' . $text . '</span>';
	}
