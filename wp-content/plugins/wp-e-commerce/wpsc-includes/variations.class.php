...
class nzshpcrt_variations {  
  ...
  function list_variations($product_id = null) {
	  /*echo "<script type='javascript'>alert('stuff7')</script>";*/
  	global $wpdb;
    $options = "";
    ...
    foreach((array)$variations as $variation) {
      ...
      *  get the list of variation values
      ...
      if(($product_id > 0) && ($check_variation > 0)) {
				...
				//if($variation['variation_association'] == 'null')
...
/*				echo "<script>alert('".$variation['variation_association']."')</script>";*/
...
/*				if($variation_value['variation_association'] == 0)
					echo "<script>alert('".$variation_value['name'].": 0')</script>"; 
				else
					echo "<script>alert('NULL')</script>"; */
  ...
  /**
   * This method imitates the array sent by POST one would expect when adding a variation, 
     it is used for the ajax code.
   */
  ...
  function add_to_existing_product($product_id,$variation_list) {
	  /*echo "<script type='javascript'>alert('stuff5')</script>";*/
     global $wpdb;
  ...
  function variations_grid_view($product_id, $variation_ids = null,  $variation_values = null, $selected_price = null, $limited_stock = null, $current_stock=null) {
    global $wpdb;
	  /*echo "<script type='javascript'>alert('stuff4')</script>";*/
...
/*echo "<script type='javascript'>alert('stuff3')</script>";
*/		// Edit or update the variation values association table
  ...
  */
  function update_variation_values($product_id, $variation_values) {
	  /*echo "<script>alert('stuff1')</script>";*/
    global $wpdb;
    $product_id = (int)$product_id;
