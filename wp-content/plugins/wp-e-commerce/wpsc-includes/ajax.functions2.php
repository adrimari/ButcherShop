...
function wpsc_add_to_cart() {
	...
	} else if (!isset($_POST['units_grams'.$product_id]) && !isset($_POST['units_kilograms'.$product_id]) && !isset($_POST['units_ounces'.$product_id]) && !isset($_POST['units_pounds'.$product_id])){
		$provided_parameters['custom_units'] = 0;
		foreach((array)$_POST['variation'] as $key => $variation) {
			$provided_parameters['variation_values'][(int)$key] = (int)$variation;
	...
	if (isset($_POST['total_price'.$product_id])) {
		...
		if (isset($_POST['weight']) && $_POST['weight'] == "add") {
			if ($_POST['total_price'.$product_id] != 0) {
				...
				$cart_messages[] = __('You cannot add an item of value R0.00 to your cart', 'wpsc');
			}
		}
		else if (isset($_POST['weight']) && $_POST['weight'] == "update")
...
function update_cart() {
	...
	if (isset($_POST['quantity'.$key]) && $_POST['wpsc_update_quantity'.$key] == 'true') {
    	if ($_POST[$qty] != 0) {
    		...
    		$wpsc_cart->get_cart_item_by_key($key)->total_price = $wpsc_cart->get_cart_item_by_key($key)->unit_price * $_POST[$qty];
    	...
    	if ($_POST[$qty] == 0)
        	...
        	$wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price = $wpsc_cart->get_cart_item_by_key($_POST['key'])->unit_price * $_POST[$qty];
	...
	if(is_numeric($_POST['key'])) {
		...
		if($_POST['custom_wpsc_quantity_update'.$cart_item->product_id] > 0) {
			...
			if (isset($_POST['unit_price'.$cart_item->product_id])) {
				...
				$parameters['total_price'] = $_POST['unit_price'.$cart_item->product_id] * $parameters['quantity'];// (int)$_POST['quantity'];
			} else {
