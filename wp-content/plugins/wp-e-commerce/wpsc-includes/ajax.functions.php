...
function wpsc_add_to_cart() {
	...
	$default_parameters['old_quantity'] = (int)abs($_POST['custom_wpsc_quantity_update'.$product_id]);
	...
	$default_parameters['custom_units'] = null;
	$default_parameters['units_grams'] = null;
	$default_parameters['units_kilograms'] = null;
	$default_parameters['units_ounces'] = null;
	$default_parameters['units_pounds'] = null;
	$default_parameters['total_price'] = null;
	/// sanitise submitted values
	$product_id = (int)$_REQUEST['product_id'];
	
	$sql = $wpdb->get_row("SELECT * FROM `wp_wpsc_product_list` WHERE `id` = ".$product_id);
	if($sql->custom_units == 0) {
		$provided_parameters['custom_units'] = 0;
		foreach((array)$_POST['variation'] as $key => $variation) {
			$provided_parameters['variation_values'][(int)$key] = (int)$variation;
		}
	} else if (!isset($_POST['units_grams'.$product_id]) && !isset($_POST['units_kilograms'.$product_id])){
		$provided_parameters['custom_units'] = 0;
		
		foreach((array)$_POST['variation'] as $key => $variation) {
			$provided_parameters['variation_values'][(int)$key] = (int)$variation;
			//subsort($provided_parameters['variation_values'],)
		}
	} else
		$provided_parameters['custom_units'] = 1;

	//exit('<pre>'.print_r($_POST, true).'</pre>');
	if($_POST['custom_wpsc_quantity_update'.$product_id] > 0) {
		//$provided_parameters['old_quantity'] = abs($provided_parameters['quantity']);
		$provided_parameters['quantity'] = $_POST['custom_wpsc_quantity_update'.$product_id];

	} 
	
	if (isset($_POST['wpsc_quantity_update'.$product_id])) {
		$wpsc_cart->remove_item($_POST['key']);
		if(is_numeric($_POST['wpsc_quantity_update'.$product_id])){
			//$provided_parameters['old_quantity'] = abs($provided_parameters['quantity']);
			$provided_parameters['quantity'] = $_POST['wpsc_quantity_update'.$product_id];
		} //else {
			//$provided_parameters['quantity'] = "20";//(int)$_POST['wpsc_quantity_update'.$product_id];
		//}
	}
	
/**************************************************/

	$tot = "total_price" . $product_id;
	$tot_var = "total_price_var" . $product_id;
	if (isset($_POST[$tot]))
		$provided_parameters['total_price'] = str_replace(",", "",(float)$_POST[$tot]);
	else 
		$provided_parameters['total_price'] = $_POST[$tot_var];
	
	if (isset($_POST['units_grams'.$product_id]))
		$provided_parameters['units_grams'] = $_POST['units_grams'.$product_id];
	else 
		$provided_parameters['units_grams'] = 0;
	
	if (isset($_POST['units_kilograms'.$product_id]))
		$provided_parameters['units_kilograms'] = $_POST['units_kilograms'.$product_id];
	else 
		$provided_parameters['units_kilograms'] = 0;
		
	
/*	if (isset($_POST['units_kilograms'.$product_id]) && isset($_POST['units_grams'.$product_id])) {
		$provided_parameters['unit_price'] = $sql->price;
		$provided_parameters['total_price'] = ($_POST['units_kilograms'.$product_id] + ($_POST['units_grams'.$product_id] / 1000)) * $provided_parameters['quantity'] * $provided_parameters['unit_price'] ;
	}*/
	//  exit('<pre>'.print_r($_POST, true).'</pre>');
/*	if($_POST['is_customisable'] == 'true') {
	...
	//if ($parameters['variation_values'] == null) {
	if (isset($_POST['total_price'.$product_id])) {
		//if (isset($_POST['weight']) && $_POST['weight'] == "add") {
			//$state = $wpsc_cart->set_item($product_id,$parameters);
			/*echo "<script>document.getElementById(subtotal1).innerHTML = 'stuff1'</script>";*/
		//} else if (isset($_POST['weight']) && $_POST['weight'] == "update") {
			//$state = $wpsc_cart->set_item($product_id,$parameters);
			/*echo "<script>document.getElementById(subtotal1).innerHTML = 'stuff2'</script>";*/
		//}
		if (isset($_POST['weight']) && $_POST['weight'] == "add") {
			if ($_POST['total_price'.$product_id] != 0) {
				$state = $wpsc_cart->set_item_custom($product_id,$parameters);
			} else {
				$cart_messages[] = __('<font color=\'#ac1e1e\'>You cannot add an item of value R0.00 to your cart</font>', 'wpsc');
			}
		}
		else if (isset($_POST['weight']) && $_POST['weight'] == "update")
			$state = $wpsc_cart->set_item_update_custom($product_id,$parameters, $_POST['key']);
		//else 
			//$state = $wpsc_cart->set_item($product_id,$parameters);
	} else if (isset($_POST['total_price_var'.$product_id])){
		if (isset($_POST['prepacked']) && $_POST['prepacked'] == "add") {
			if ($_POST['total_price_var'.$product_id] != 0) {
				$state = $wpsc_cart->set_item_variation($product_id,$parameters);
			} else {
				//$s = $_POST['variation1'];
				$cart_messages[] = __('<font color=\'#ac1e1e\'>You cannot add an item of value R0.00 to your cart</font>', 'wpsc');
			}
		}
		else if (isset($_POST['prepacked']) && $_POST['prepacked'] == "update")
			$state = $wpsc_cart->set_item_update_variation($product_id,$parameters, $_POST['key']);
		//else 
			//$state = $wpsc_cart->set_item_update_variation($product_id,$parameters, $_POST['key']);
//$state = $wpsc_cart->set_item($product_id,$parameters); 
	}
...
/*	if (!isset($provided_parameters['variation_values'])) {
		$state = $wpsc_cart->set_item($product_id,$parameters,9999,true);
	} else {
		$state = $wpsc_cart->set_item($product_id,$parameters); 
	}*/
	//$state = true;
	...
	if($state == true) {
		...
		if($parameters['quantity'] <= 0) {
			$cart_messages[] = __('<font color=\'#ac1e1e\'>Sorry, but you cannot add zero items to your cart</font>', 'wpsc');
...
if($_REQUEST['wpsc_ajax_action'] == 'update_cart') {
	add_action('init', 'update_cart');
}

function update_cart() {
	global $wpsc_cart;
    /*$str = "wpsc_quantity_update".$wpsc_cart->cart_item->product_id;
    echo "<script>alert('".$_POST["wpsc_quantity_update".$wpsc_cart->cart_item->product_id]."')</script>"; */
	//if ($_POST['wpsc_quantity_update6'] == 0)
		//$wpsc_cart->remove_item(2);
	//else {
	...
	$id = $_POST['prod_id'];
	$qty = 'quantity'.$key;
	if (isset($_POST['quantity'.$key]) && $_POST['wpsc_update_quantity'.$key] == 'true') {
    	if ($_POST[$qty] != 0) {
    		$price = str_replace(",", "", substr(wpsc_cart_total(),3)) - $wpsc_cart->get_cart_item_by_key($key)->total_price;
    		$wpsc_cart->get_cart_item_by_key($key)->total_price = $wpsc_cart->get_cart_item_by_key($key)->unit_price * ceil((int)$_POST[$qty]);
        	$wpsc_cart->get_cart_item_by_key($key)->quantity = $_POST[$qty];
        	$wpsc_cart->subtotal = $price + $wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price;
    	} else
    		$wpsc_cart->remove_item($key);
   		//$c_item = $wpsc_cart->get_cart_item_by_key($key);
    	//$c_item->quantity = 444;
	} else if (isset($_POST['quantity'.$key]) && $_POST['custom_wpsc_update_quantity'] == 'true') {
    	if ($_POST[$qty] == 0)
    		$wpsc_cart->remove_item($_POST['key']);
    	else {
    		$price = str_replace(",", "", substr(wpsc_cart_total(),3)) - $wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price;
        	$wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price = $wpsc_cart->get_cart_item_by_key($_POST['key'])->unit_price * ceil((int)$_POST[$qty]);
        	$wpsc_cart->get_cart_item_by_key($_POST['key'])->quantity = $_POST[$qty];
        	$wpsc_cart->subtotal = $price + $wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price; //$wpsc_cart->wpsc_cart_total() + $wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price;
    	}


	//$url = home_url( '/' )."?page_id=11";
    	//header('Location: $url');
/*		echo "<script>document.location.href='".$url."'</script>"; */
     
        //$wpsc_cart->get_cart_item_by_key($key)->total_price = 444;//$wpsc_cart->cart_item->total_price * $_POST["wpsc_quantity_update".$key];
        //$wpsc_cart->get_cart_item_by_key($_POST['key'])->quantity = 555;//$_POST["wpsc_quantity_update".$key];
        
    //} else if (isset($_POST['wpsc_update_quantity']) && $_POST['wpsc_update_quantity'] == 'true') {
        //$wpsc_cart->cart_item->total_price = $wpsc_cart->cart_item->total_price * $_POST["wpsc_quantity_update".$key];
	//$wpsc_cart->cart_item->quantity = $_POST["quantity".$key];	
...
//else {
//	    $old_total = $wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price;
//	
//	    if ($_POST['units_grams'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id])
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->units_grams = $_POST['units_grams'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id];
//	    else
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->units_grams = 0;
//		if ($_POST['units_kilograms'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id])
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->units_kilograms = $_POST['units_kilograms'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id];
//	    else
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->units_kilograms = 0;
//		if ($_POST['units_ounces'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id])
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->units_ounces = $_POST['units_ounces'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id];
//	    else
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->units_ounces = 0;
//		if ($_POST['units_pounds'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id])
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->units_pounds = $_POST['units_pounds'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id];
//	    else
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->units_pounds = 0;
//			
		//if($_POST['total_price'.$wpsc_cart->cart_item->product_id])
//			$wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price = $_POST['total_price'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id];
		//else
			//$wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price = 
			
//	    $wpsc_cart->get_cart_item_by_key($_POST['key'])->custom_weight = 0.00;
//	    if ($_POST['units_grams'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id] != "")
//		    $wpsc_cart->get_cart_item_by_key($_POST['key'])->custom_weight += $_POST['units_grams'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id] / 1000;				
//	    if ($_POST['units_kilograms'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id] != "")
//		    $wpsc_cart->get_cart_item_by_key($_POST['key'])->custom_weight += $_POST['units_kilograms'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id];				
//	    if ($_POST['units_ounces'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id] != "")
//		    $wpsc_cart->get_cart_item_by_key($_POST['key'])->custom_weight += $_POST['units_ounces'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id];				
//	    if ($_POST['units_pounds'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id] != "")
//		    $wpsc_cart->get_cart_item_by_key($_POST['key'])->custom_weight += $_POST['units_pounds'.$wpsc_cart->get_cart_item_by_key($_POST['key'])->product_id];
//	    $wpsc_cart->get_cart_item_by_key($_POST['key'])->custom_weight .= " Kg";
	
/*	echo "<script>alert('".$wpsc_cart->subtotal."')</script>";
	echo "<script>alert('".$old_total."')</script>";
	echo "<script>alert('".$wpsc_cart->cart_item->total_price."')</script>";*/
//	    $wpsc_cart->subtotal = $wpsc_cart->subtotal - $old_total + $wpsc_cart->get_cart_item_by_key($_POST['key'])->total_price;
/*	echo "<script>alert('".$wpsc_cart->subtotal."')</script>";*/
    //}
	//}
	//$url = home_url( '/' )."?page_id=11";
...
function wpsc_update_item_quantity() {
	global $wpdb, $wpsc_cart, $wpsc_theme_path;
	$key = (int)$_POST['key'];
 	$cart_item = $wpsc_cart->get_cart_item_by_key($key);
	if(is_numeric($_POST['key'])) {
		//$key = (int)$_POST['key'];
		if($_POST['custom_wpsc_quantity_update'.$cart_item->product_id] > 0) {
			//$wpsc_cart->get_cart_item_by_key($_POST['key'])->old_quantity = $wpsc_cart->get_cart_item_by_key($_POST['key'])->quantity;
			//$cart_item = $wpsc_cart->get_cart_item_by_key($key);
			// if the quantity is greater than 0, update the item;
			$parameters['quantity'] = (int)$_POST['custom_wpsc_quantity_update'.$cart_item->product_id];
			$parameters['custom_units'] = (int)$cart_item->custom_units;
			if (isset($_POST['units_grams'.$cart_item->product_id]))
				$parameters['units_grams'] = $_POST['units_grams'.$cart_item->product_id];
			else
				$parameters['units_grams'] = (int)$cart_item->units_grams;
			if (isset($_POST['units_kilograms'.$cart_item->product_id]))
				$parameters['units_kilograms'] = $_POST['units_kilograms'.$cart_item->product_id];
			else
				$parameters['units_kilograms'] = (int)$cart_item->units_kilograms;
			if (isset($_POST['units_ounces'.$cart_item->product_id]))
				$parameters['units_ounces'] = $_POST['units_ounces'.$cart_item->product_id];
			else
				$parameters['units_ounces'] = (int)$cart_item->units_ounces;
			if (isset($_POST['units_pounds'.$cart_item->product_id]))
				$parameters['units_pounds'] = $_POST['units_pounds'.$cart_item->product_id];
			else
				$parameters['units_pounds'] = (int)$cart_item->units_pounds;
			if (isset($_POST['unit_price'.$cart_item->product_id])) {
				$parameters['unit_price'] = $_POST['unit_price'.$cart_item->product_id];
				$parameters['total_price'] = $_POST['unit_price'.$cart_item->product_id] * ceil($parameters['quantity']);// (int)$_POST['quantity'];
			} else {
				$parameters['unit_price'] = (float)$_POST['total_price'.$cart_item->product_id]; //$cart_item->unit_price;
				$parameters['total_price'] = (float)$parameters['unit_price'] * (int)$_POST['custom_wpsc_quantity_update'.$cart_item->product_id];
			}
			//if (isset($_POST['custom_weight']))
				//$parameters['custom_weight'] = $_POST['custom_weight'];
			//else
				$parameters['custom_weight'] = 0;
				if ($parameters['units_grams'])
					$parameters['custom_weight'] += (float)($parameters['units_grams'] / 1000);
				if ($parameters['units_kilograms'])	
					$parameters['custom_weight'] += (float)$parameters['units_kilograms'];
				if ($parameters['units_ounces'])	
					$parameters['custom_weight'] += (float)$parameters['units_ounces'];
				if ($parameters['units_ounces'])	
					$parameters['custom_weight'] += (float)$parameters['units_pounds'];
			//if (isset($_POST['weight']))
				//$parameters['weight'] = $_POST['weight'];
			//else
				$parameters['weight'] = $parameters['custom_weight'];//(float)$cart_item->weight;

			//$wpsc_cart->edit_item($key, $parameters);
		}
		else if($_POST['wpsc_quantity_update'.$cart_item->product_id] > 0)
		{
			$parameters['quantity'] = (int)$_POST['wpsc_quantity_update'.$cart_item->product_id];
		} else {
...
function wpsc_submit_checkout() {
  ...
  //echo "break redirect";
	...
	$email_address=$_POST["collected_data"][8];

	$sql = "SELECT Count(*) AS Total FROM `".WPSC_TABLE_CHECKOUT_FORMS."` LEFT JOIN `".WPSC_TABLE_SUBMITED_FORM_DATA."` ON `".WPSC_TABLE_CHECKOUT_FORMS."`.id = `".WPSC_TABLE_SUBMITED_FORM_DATA."`.`form_id` WHERE `".WPSC_TABLE_SUBMITED_FORM_DATA."`.`form_id`=8 AND `".WPSC_TABLE_SUBMITED_FORM_DATA."`.`value`='".$email_address."' AND `".WPSC_TABLE_SUBMITED_FORM_DATA."`.`Created` LIKE '".date("Y-m-d",time())."%' ";
	$checkouts_today = $wpdb->get_results($sql,ARRAY_A) ;
	$total_checkouts_total = $checkouts_today[0]["Total"];

   //Prevent more than 5 submissions per email in a day
   if ($_SERVER["HTTP_HOST"]!="localhost")
		if ($total_checkouts_total>5) {
			//$_SESSION['prevent_checkout']=true;
			//$_SESSION['wpsc_checkout_misc_error_messages'][]="You are unable to proceed in processing any online transactions for today. <br/>Please contact The Butcher Shop & Grill for a telephonic purchase.";
			//return;
		}
 ...
 // exit('valid >'.$is_valid);
  if(array_search($submitted_gateway,$selected_gateways) !== false) {
	...
	if($is_valid == true || $_GET['gateway'] == 'noca') {
		...
		//3d secure check
		if (isset($_SESSION["force_3dsecureOff"]) || (get_option('3dsecure_status') != 'active') || (get_option('3dsecure_status') == 'active' && isset($_SESSION["ThreeDSecure_TransactionID"]))) {
			$wpdb->query($sql);
		}
...
// execute on POST and GET
if($_REQUEST['wpsc_action'] == 'submit_checkout' || isset($_SESSION["POST_backup"])) {
	if (isset($_SESSION["POST_backup"])) {
		$_POST=array_merge($_POST,$_SESSION["POST_backup"]);
		unset($_SESSION["POST_backup"]); //very important to prevent redirect loop
	} else {
