...
/***************************************************************/
function get_variation_names($id) {
	global $wpdb;
	$query = $wpdb->get_results("SELECT `val`.`name` FROM `".WPSC_TABLE_VARIATION_VALUES."` AS `val` LEFT JOIN `".WPSC_TABLE_PRODUCT_VARIATIONS."` AS `var` ON `val`.`variation_id` = `var`.`id` WHERE `var`.`id` = {$id}",ARRAY_A) ;
...
function calculate_product_price($product_id, $variations = false, $no_special=false) {
	/*echo "<script>alert('calculate_product_price')</script>";*/
	global $wpdb;
	if(is_numeric($product_id)) {
		...
		if($variation_count >= 1) {
			...
			if(get_variation_names($variation_ids[0])) {
				/*echo "<script>alert('get_variation_name = custom')</script>";*/
				$priceandstock_id = $wpdb->get_var("SELECT `priceandstock_id` FROM `".WPSC_TABLE_VARIATION_COMBINATIONS."` WHERE `product_id` = {$product_id} AND `value_id` = {$variation_ids[0]}");
				/*echo "<script>alert('".$priceandstock_id."')</script>";*/
				...
				/*echo "<script>alert('variation_select_{$product_id}_{$variation_ids[0]}')</script>";*/
				echo $_POST['variation_select_{$variation_ids[0]}'];
				if($_POST['variation_select_{$variation_ids[0]}']) {
					echo "<script>alert('variation_select_{$variation_ids[0]}')</script>";
				}
			}
...
function wpsc_add_product($product_values) {
    global $wpdb;
		...
		$insertsql = "UPDATE `".WPSC_TABLE_PRODUCT_VARIATIONS."` SET `unit` = 'stuff' WHERE `id` = 1";
		...
		if($wpdb->query($insertsql)) {  
		  // if we succeeded, we have a product id, we wants it for the next stuff
			...
			//if($product_values[] != null) {
				$insertsql = "INSERT INTO `wpsc_product_variations` SET `unit` = `stuff` WHERE `id` = 1";
				$wpdb->query($insertsql);
			//}
