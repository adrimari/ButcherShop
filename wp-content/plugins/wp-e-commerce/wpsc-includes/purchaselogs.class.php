...
class wpsc_purchaselogs_items{
	...
	var $userinfo;
	var $shippinginfo;
	var $customcheckoutfields = array("Trace"=>"trace");
	...
	function get_purchlog_details(){
		...
		foreach((array)$this->extrainfo as $key=>$value) {
			if(in_array($key,$this->customcheckoutfields)){
				$fields=array_keys((array)$this->customcheckoutfields,$key);
				$additionaldetails[$fields[0]] = array("value"=>$value);
			}
		}	
