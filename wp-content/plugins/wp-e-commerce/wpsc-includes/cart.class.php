...
//	echo 'cart shipping total'.$wpsc_cart->calculate_total_shipping();
	...
	//if ($total < $wpsc_cart->subtotal)
		//$wpsc_cart->subtotal = 666;
	}
...
function wpsc_cart_item_name() {
 ...
 /**
* cart item product variation string function, no parameters
* @return string the cart item variations
*/
function wpsc_cart_item_variation_str() {
	global $wpsc_cart;
	return htmlentities(stripslashes($wpsc_cart->cart_item->product_variation_str), ENT_QUOTES, "UTF-8");
}
 /**
...
function wpsc_cart_item_product_id() {
 ...
 /**
* cart item custom units function, no parameters
* @return string the selected quantity of items
*/
function wpsc_cart_item_custom_units() {
	global $wpsc_cart;
	return $wpsc_cart->cart_item->custom_units;
} 

function wpsc_cart_custom_weight() {
	global $wpsc_cart;
	return $wpsc_cart->cart_item->custom_weight;
} 
...
* @return string the selected quantity of items
*/
function wpsc_cart_item_units_grams() {
	global $wpsc_cart;
	return $wpsc_cart->cart_item->units_grams;
} /**
* cart item custom units function, no parameters
* @return string the selected quantity of items
*/
function wpsc_cart_item_units_kilograms() {
	global $wpsc_cart;
	return $wpsc_cart->cart_item->units_kilograms;
} /**
* cart item custom units function, no parameters
* @return string the selected quantity of items
*/
function wpsc_cart_item_units_ounces() {
	global $wpsc_cart;
	return $wpsc_cart->cart_item->units_ounces;
} /**
* cart item custom units function, no parameters
* @return string the selected quantity of items
*/
function wpsc_cart_item_units_pounds() {
	global $wpsc_cart;
	return $wpsc_cart->cart_item->units_pounds;
}
...
function wpsc_cart_item_quantity() {
	global $wpsc_cart;
	return ceil($wpsc_cart->cart_item->quantity);
}
...
function wpsc_cart_item_price($forDisplay = true) {
	global $wpsc_cart;
	/*echo "<script>alert('".$wpsc_cart->cart_item->wpsc_cart_item_count."')</script>";*/
	if($forDisplay){
		return $wpsc_cart->process_as_currency(str_replace(",", "", $wpsc_cart->cart_item->total_price));
	}else{
		return str_replace(",", "", $wpsc_cart->cart_item->total_price);
	}
}
...
/***************************************************/
	function set_item_custom($product_id, $parameters) {
		$parameters['total_price'] = str_replace(",", "", $parameters['total_price']);
		if(($parameters['quantity'] > 0) && ($this->check_remaining_quantity($product_id, $parameters['variation_values'], ceil($parameters['quantity'])) == true)) {
		...
		$this->clear_cache();
		do_action ("wpsc_cart_updated", array(&$this));
		return $status;
	}
	
	function set_item_update_custom($product_id, $parameters, $key) {
	...
	function set_item_update($product_id, $parameters) {
		$parameters['total_price'] = str_replace(",", "", $parameters['total_price']);
		if(($parameters['quantity'] > 0) && ($this->check_remaining_quantity($product_id, $parameters['variation_values'], $parameters['quantity']) == true)) {
			$new_cart_item = new wpsc_cart_item($product_id,$parameters, $this);
			$this->cart_items[] = $new_cart_item;
	...
	function set_item_variation($product_id, $parameters) {
		global $wpsc_cart;
		if(($parameters['quantity'] > 0)) {// && ($this->check_remaining_quantity($product_id, $parameters['variation_values'], $parameters['quantity']) == true)) {
			$new_cart_item = new wpsc_cart_item($product_id,$parameters, $this);
			$this->cart_items[] = $new_cart_item;
	...
	function set_item_update_variation($product_id, $parameters, $key) {
		global $wpsc_cart;
		if(($parameters['quantity'] > 0)) { // && ($this->check_remaining_quantity($product_id, $parameters['variation_values'], $parameters['quantity']) == true)) {
			$new_cart_item = new wpsc_cart_item($product_id,$parameters, $this);
			//$wpsc_cart->remove_item($key);
...
/***************************************************/
	...
	*/
	function set_item($product_id, $parameters, $total, $updater = false) {
		...
		if(($parameters['quantity'] > 0) && ($this->check_remaining_quantity($product_id, $parameters['variation_values'], $parameters['quantity']) == true)) {
			...
			if((count($this->cart_items) > 0) && ($new_cart_item->is_donation != 1)) {
				...
				foreach($this->cart_items as $key => $cart_item) {
					if ($cart_item->custom_units == 1) {
						//$this->cart_items[$key]->name = "custom";
						if ($cart_item->product_id == $new_cart_item->product_id) {
							$this->cart_items[$key]->quantity = ceil(abs($new_cart_item->quantity));
							//$this->cart_items[$key]->old_quantity = abs($new_cart_item->quantity);
							$this->cart_items[$key]->total_price = str_replace(",", "", $new_cart_item->total_price);
							$this->cart_items[$key]->refresh_item();
						} else {
							$add_item = true;
						}
					...
					else if(($cart_item->product_id == $new_cart_item->product_id) &&
					...
					($cart_item->custom_file == $new_cart_item->custom_file)) { 
						if (($cart_item->units_grams != null) ||
							($cart_item->units_kilograms != null) ||
							($cart_item->units_ounces != null) ||
							($cart_item->units_pounds != null)) {
							continue;
						}
						// if they are the same, increment the count, and break out;
						if(!$updater){
							$this->cart_items[$key]->quantity  = ceil(abs($new_cart_item->quantity));
							//$this->cart_items[$key]->total_price = $total;
						} else {
							//$this->cart_items[$key]->quantity  = abs($new_cart_item->quantity);
							//$this->cart_items[$key]->custom_weight = $updater;
							//$this->cart_items[$key]->total_price = 1000;
						}
						//$this->cart_items[$key]->refresh_item();
						$add_item = false;
						$edit_item = true;
  ...
  function edit_item($key, $parameters) {
    if(isset($this->cart_items[$key])) {
			...
			$quantity = ceil(abs($parameters['quantity']));// - abs($this->cart_items[$key]->quantity);
  ...
  function empty_cart($fromwidget = true) {
		...
		do_action('wpsc_empty_cart', array(&$this));
  }
	...
	*/
	function calculate_subtotal($for_shipping = false) {
		...
		if($for_shipping == true ) {
			...
			$total = 0;
/*			echo "<script>alert('this->cart_items: '".$this->cart_items."')</script>";*/
  ...
  function get_cart_item_by_key($key) {
	//global $wpsc;
	if(isset($this->cart_items[$key])) {
		return $this->cart_items[$key];
	}
/*	$i = 0;
	foreach($this->cart_items[$mykey] as $key => $cart_item) {
		if ($cart_item == $mykey)
			return $this->cart_item;
		$i++;
	}*/	
...
// each cart item contains a reference to the cart that it is a member of
	...
	var $custom_units = 0;
	var $units_grams = 0.00;
	var $units_kilograms = 0.00;
	var $units_ounces = 0.00;
	var $units_pounds = 0.00;
	var $price = 0.00;
	var $custom_weight = 0.00;
	...
	*/
	function update_item($quantity) {
		//$key = wpsc_the_cart_item_key();
		//$this->old_quantity = $this->quantity;
		$this->quantity = (int)ceil(abs($quantity));
		//if ($this->quantity == 0)
...
function refresh_item() 
{
	...
	$product = $wpdb->get_row("SELECT * FROM `wp_wpsc_product_list` WHERE `id` = '{$this->product_id}' LIMIT 1", ARRAY_A);
	$priceandstock_id = 0;

	if(defined('WPSC_ADD_DEBUG_PAGE') && (constant('WPSC_ADD_DEBUG_PAGE') == true)) {
		$this->product_data = $wpdb->get_row("SELECT * FROM `wp_wpsc_product_list` WHERE `id` = '{$this->product_id}' LIMIT 1", ARRAY_A);
	}
	
	//$this->custom_weight = "SELECT * FROM `".WPSC_TABLE_VARIATION_VALUES."` WHERE `id` IN ('".implode("','",$this->variation_values)."')";
	if(count($this->variation_values) > 0) {
		// if there are variations, get the price of the combination and the names of the variations.
		$variation_data = $wpdb->get_results("SELECT * FROM `wp_wpsc_variation_values` WHERE `id` IN ('".implode("','",$this->variation_values)."')", ARRAY_A);
...
//$test=$variation_ids;
		...
		//$this->custom_weight = "SELECT `priceandstock_id` FROM `".WPSC_TABLE_VARIATION_COMBINATIONS."` WHERE `product_id` = '{$this->product_id}' AND `value_id` IN ( '".implode("', '",$this->variation_values )."' ) AND `all_variation_ids` IN('$variation_id_string') GROUP BY `priceandstock_id` HAVING COUNT( `priceandstock_id` ) = '".count($this->variation_values)."' LIMIT 1";
		$priceandstock_id = $wpdb->get_var("SELECT `priceandstock_id` FROM `wp_wpsc_variation_combinations` WHERE `product_id` = '{$this->product_id}' AND `value_id` IN ( '".implode("', '",$this->variation_values )."' ) AND `all_variation_ids` IN('$variation_id_string') GROUP BY `priceandstock_id` HAVING COUNT( `priceandstock_id` ) = '".count($this->variation_values)."' LIMIT 1");	
		$priceandstock_values = $wpdb->get_row("SELECT * FROM `wp_wpsc_variation_properties` WHERE `id` = '{$priceandstock_id}' LIMIT 1", ARRAY_A);
		$price = str_replace(",", "",$priceandstock_values['price']);
		$weight = $priceandstock_values['weight'] * $this->quantity;//$wpdb->get_var("SELECT `weight` FROM `wp_wpsc_product_list` WHERE `id` = '{$this->product_id}' LIMIT 1");	
		//$weight = $weight * $this->quantity;
		//wpsc_convert_weights($priceandstock_values['weight'], $priceandstock_values['weight_unit']);
...
/*********************************************************/
		...
		if (!$this->unit_price)
			$price = str_replace(",", "",number_format((float)$this->total_price,2));// -  number_format((float)$sale_discount,2);
		else
			$price = str_replace(",", "",number_format((float)$this->unit_price,2));// -  number_format((float)$sale_discount,2);
		
		if ($this->custom_units == 1) {// && $_POST['units_grams'.$this->product_id] != 0 && $_POST['units_kilograms'.$this->product_id] != 0 && $_POST['units_ounces'.$this->product_id] != 0 && $_POST['units_pounds'.$this->product_id] != 0 ) 
			if (isset($_POST['wpsc_update_quantity']) && $_POST['wpsc_update_quantity'] == 'true' && $_POST['key'] != -1)
				$this->units_grams = $_POST['units_grams'.$this->product_id]; //$this->units_grams;
			else if ($_POST['units_grams'.$this->product_id] != "" || $_POST['units_grams'.$this->product_id] != "0" && $_POST['key'] != -1) {
				if ($_POST['units_grams'.$this->product_id] < $product['min_grams'] || $_POST['units_grams'.$this->product_id] > $product['max_grams'] && $_POST['key'] != -1) {
					$this->units_grams = 0.00; //"Value must be between ".$product['min_grams']." and ".$product['max_grams']." grams.";
					//$price = 0.00;
				} else {
					$this->units_grams = $_POST['units_grams'.$this->product_id];
				}
			}
			
			if (isset($_POST['wpsc_update_quantity']) && $_POST['wpsc_update_quantity'] == 'true' && $_POST['key'] != -1)
				$this->units_kilograms = $_POST['units_kilograms'.$this->product_id]; //$this->units_kilograms;
			else if ($_POST['units_kilograms'.$this->product_id] != "" || $_POST['units_kilograms'.$this->product_id] != "0" && $_POST['key'] != -1) {
				if ($_POST['units_kilograms'.$this->product_id] < $product['min_kilograms'] || $_POST['units_kilograms'.$this->product_id] > $product['max_kilograms'] && $_POST['key'] != -1) {
					$this->units_kilograms = 0.00; //"Value must be between ".$product['min_kilograms']." and ".$product['max_kilograms']." kilograms.";
					//$price = 0.00;
				} else {
					$this->units_kilograms = $_POST['units_kilograms'.$this->product_id];
				}
			}
			
			if (isset($_POST['wpsc_update_quantity']) && $_POST['wpsc_update_quantity'] == 'true' && $_POST['key'] != -1)
				$this->units_ounces = $_POST['units_ounces'.$this->product_id]; //$this->units_ounces;
			else if ($_POST['units_ounces'.$this->product_id] != "" || $_POST['units_ounces'.$this->product_id] != "0" && $_POST['key'] != -1) {
				if ($_POST['units_ounces'.$this->product_id] < $product['min_ounces'] || $_POST['units_ounces'.$this->product_id] > $product['max_ounces'] && $_POST['key'] != -1) {
					$this->units_ounces = 0.00; //"Value must be between ".$product['min_ounces']." and ".$product['max_ounces']." ounces.";
					//$price = 0.00;
				} else {
					$this->units_ounces = $_POST['units_ounces'.$this->product_id];
				}
			}
			if (isset($_POST['wpsc_update_quantity']) && $_POST['wpsc_update_quantity'] == 'true' && $_POST['key'] != -1)
				$this->units_pounds = $_POST['units_pounds'.$this->product_id]; //$this->units_pounds;
			else if ($_POST['units_pounds'.$this->product_id] != "" || $_POST['units_pounds'.$this->product_id] != "0" && $_POST['key'] != -1) {
				if ($_POST['units_pounds'.$this->product_id] < $product['min_pounds'] || $_POST['units_pounds'.$this->product_id] > $product['max_pounds'] && $_POST['key'] != -1) {
					$this->units_pounds = 0.00; //"Value must be between ".$product['min_pounds']." and ".$product['max_pounds']." pounds.";
					//$price = 0.00;
				} else {
					$this->units_pounds = $_POST['units_pounds'.$this->product_id];
				}
			}
			//$this->custom_weight = null;	
			
			if ($this->units_grams) {
				$this->custom_weight += $this->units_grams / 1000;				
			}
			if ($this->units_kilograms) {
				$this->custom_weight += $this->units_kilograms;	
			}				
			if ($this->units_ounces) {
				$this->custom_weight += $this->units_ounces;
			}
			if ($this->units_grams) {
				$this->custom_weight += $this->units_pounds;
			}
				
			$this->weight = $this->custom_weight;
			//$price = $this->weight * $this->price;
		}
/*			}
		...
		if ($levels != '') {
			foreach((array)$levels['quantity'] as $key => $qty) {
				if ($this->quantity >= $qty) {
					$unit_price = str_replace(",", "", $levels['table_price'][$key]);
					if ($unit_price != '') {
						$price = $unit_price;
					}
				}
	...
	if(count($variation_names) > 0) {
		$product_variation_str .= implode(", ",$variation_names);
		$this->weight = $weight;
	} else if ($product['custom_units']) {

		$custom_weight=null;
		$unit_weight_type=null;
		if ($this->custom_weight<1) {
			$custom_weight=$this->custom_weight*1000;
			$unit_weight_type="g";
		} else {
		...
		$product_variation_str .= "{$custom_weight} {$unit_weight_type}";
		$product_variation_str .= " (R{$price})";
		$this->weight = $this->custom_weight;
	}
	
	$this->product_name = $product_name;
	$this->product_variation_str = $product_variation_str;
	...
	if($this->is_donation == 1) {
		(float)$this->unit_price = str_replace(",", "", number_format((float)$this->provided_price,2));
	} else if ($this->custom_units) {
		(float)$this->unit_price = str_replace(",", "", number_format((float)$price,2)); //$this->units_grams / 1000 * 
		//$this->unit_price = number_format($price / abs($this->quantity),2);
	} else {
		 (float)$this->unit_price = str_replace(",", "", number_format((float)$price,2));
	}
	...
	//$this->weight = $this->custom_weight;
	//$this->weight = $weight;
	if (!$this->custom_units || ($this->units_grams == 0 && $this->units_kilograms == 0 && $this->units_ounces == 0 && $this->units_pounds == 0))
		 (float)$this->total_price = str_replace(",", "", number_format($this->unit_price * ceil(abs($this->quantity)),2));//$this->total_price / $this->old_quantity * $this->quantity;
	else if (!$this->custom_units && $this->units_grams == '' && $this->units_kilograms == '' && $this->units_ounces == '' && $this->units_pounds == '')
		$this->total_price = 999;
	else 
		(float)$this->total_price = str_replace(",", "", number_format($this->unit_price * ceil(abs($this->quantity)),2));//$this->unit_price;
	...
	$this->price = str_replace(",", "",$this->total_price);
	$this->update_claimed_stock();
	...
	*/
	function save_to_db($purchase_log_id) {
		...
		if ($this->product_variation_str)
			$variation_str=" - {$this->product_variation_str}";
		
		if ($this->custom_units) {
			$wpdb->query($wpdb->prepare("INSERT INTO `".WPSC_TABLE_CART_CONTENTS."` (`prodid`, `name`, `purchaseid`, `price`, `pnp`,`tax_charged`, `gst`, `quantity`, `donation`, `no_shipping`, `custom_message`, `files`, `meta`) VALUES ('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%d', '0', '%s', '%s', NULL)", $this->product_id, $this->product_name."{$variation_str}", $purchase_log_id, $this->unit_price, (float)$shipping, (float)$tax, (float)$tax_rate, $this->quantity, $this->is_donation, $this->custom_message, serialize($this->custom_file)));
		} else {
			$wpdb->query($wpdb->prepare("INSERT INTO `".WPSC_TABLE_CART_CONTENTS."` (`prodid`, `name`, `purchaseid`, `price`, `pnp`,`tax_charged`, `gst`, `quantity`, `donation`, `no_shipping`, `custom_message`, `files`, `meta`) VALUES ('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%d', '0', '%s', '%s', NULL)", $this->product_id, $this->product_name."{$variation_str}", $purchase_log_id, $this->unit_price, (float)$shipping, (float)$tax, (float)$tax_rate, $this->quantity, $this->is_donation, $this->custom_message, serialize($this->custom_file)));
		}
