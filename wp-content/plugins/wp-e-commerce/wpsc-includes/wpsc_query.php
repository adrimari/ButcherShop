...
* @return string - the class of the selected category
*/
function wpsc_current_category_name_by_id($id) {
	global $wpdb;
	$sql = $wpdb->get_var("SELECT name FROM `wp_wpsc_product_categories` WHERE `id` = " . $id ."");
	return $sql; //$wpsc_query->product[$id];
}

/**
* Item weight unit, finds the weight unit of the item
* @return string - the weight unit of the item
*/
function wpsc_weight_unit($id) {
	global $wpdb;
	$sql = $wpdb->get_var("SELECT `weight_unit` FROM `wp_wpsc_product_categories` WHERE `id` = " . $id ."");
	return $sql; //$wpsc_query->product[$id];
}
...
function wpsc_edit_the_product_link( $link = null, $before = '', $after = '', $id = 0 ) {
	...
	if($current_user->{$table_prefix . 'capabilities'}['administrator'] == 1) {
		$output = $before . "<a href='{$siteurl}/wp-admin/admin.php?page=wpsc-edit-products&amp;product_id={$product_id}'>" . $link . "</a>" . $after;
	}
	return $output;
}
...
/**
* wpsc product in cart function
* @return boolean - true if the product is in the cart, otherwise false
*/
function in_cart($product_id, $in_cart = false) {
	global $wpsc_cart;
	while (wpsc_have_cart_items()) : wpsc_the_cart_item();
		if (wpsc_cart_item_product_id() == $product_id) {
			$in_cart = true;
			break;
		}
	endwhile;
...
function wpsc_the_variation_name() {
	global $wpsc_query;
/*	$name = stripslashes($wpsc_query->variation['name']);
	echo "<script type='text/javascript'>alert('".$name." in function')</script>";*/
...
/**
* wpsc the variation unit function
* @return string - the variation unit
*/
function wpsc_the_variation_unit() {
	global $wpsc_query;
	return $wpsc_query->variation['unit'];
}
...
class WPSC_query {
	...
	*/
	function get_variation_groups() {
		global $wpdb;
		$this->variation_groups = $wpdb->get_results("SELECT `v`.`id` AS `variation_id`,`v`.`name`,`v`.`unit` 	FROM `".WPSC_TABLE_VARIATION_ASSOC."` AS `a` JOIN `".WPSC_TABLE_PRODUCT_VARIATIONS."` AS `v` ON `a`.`variation_id` = `v`.`id` WHERE `a`.`type` IN ('product') AND `a`.`associated_id` IN ('{$this->product['id']}')", ARRAY_A);
	...
	function get_first_variations() {
		...
		foreach((array)$this->variation_groups as $variation_group) {
		  $variation_id = $variation_group['variation_id'];
		 	$query="SELECT `v`.*, `pv`.`unit` FROM `".WPSC_TABLE_VARIATION_VALUES_ASSOC."` AS `a` JOIN `".WPSC_TABLE_PRODUCT_VARIATIONS."` AS `pv` ON `pv`.`id`=`a`.`variation_id` JOIN `".WPSC_TABLE_VARIATION_VALUES."` AS `v` ON `a`.`value_id` = `v`.`id` WHERE `a`.`product_id` IN ('{$this->product['id']}') AND `a`.`variation_id` IN ('$variation_id') AND `a`.`visible` IN ('1') ORDER BY `v`.`id` ASC";
			$this->all_associated_variations[$variation_id] = $wpdb->get_results($query, ARRAY_A);
