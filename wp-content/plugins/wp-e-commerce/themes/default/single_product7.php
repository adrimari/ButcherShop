...
<?php //****************************** ?>
				...
				<?php if(wpsc_product_external_link(wpsc_the_product_id()) != '') {
					...
					$action =  htmlentities(get_option('shopping_cart_url')."&update=true",ENT_QUOTES); 
				} ?>
				...
				<?php if ($query->custom_units == 1) { ?>
                    ...
                    <?php if (!isset($_GET['edit']) && isset($_GET['key'])) : ?>
                    	<div class="edit_prod" onclick="enable_edit('<?php echo wpsc_the_product_id(); ?>')" title="Edit this product">[Edit this product...]</div><!-- .edit_prod -->	
					<?php endif; ?>
                    <?php if (isset($_GET['key'])) : ?>
                    	<div class="add_prod"><a href="<?php echo get_site_url()."/?page_id=".$_GET['page_id']."&category=".$_GET['category']."&product_id=".$_GET['product_id']."&custom=".$_GET['custom']; ?>" title="Add new Product">[Add a new product...]</a></div><!-- .add_prod -->
                    <?php endif; ?>
                    </div>
					<div id="custom<?php echo wpsc_the_product_id(); ?>" class="custom" style="display:<?php echo ($_GET['custom'] == '1') ? "block" : ($wpsc_cart->get_cart_item_by_key($_GET['key']) && $wpsc_cart->get_cart_item_by_key($_GET['key'])->custom_units == 1) ? "block" : "none"; ?>; width: 164px;">
                        ...
                        <form class='product_form'  enctype="multipart/form-data" action="<?php echo str_replace("amp;","",$action); ?>" method="post" name="custom_product_<?php echo wpsc_the_product_id(); ?>" id="custom_product_<?php echo wpsc_the_product_id(); ?>">
                 			<input type="hidden" name="custom_product_<?php echo wpsc_the_product_id(); ?>_single" id="custom_product_<?php echo wpsc_the_product_id(); ?>_single" value="true" />
								...
								<tr>
									...
									<td width="20px">
										...
										<input size='3' maxlength='3' type='text' class='wpsc_select_variation' 
                                            ...
                                            onkeydown="clear1('<?php echo wpsc_the_product_id(); ?>','units_kilograms<?php echo wpsc_the_product_id(); ?>','total_kilograms<?php echo wpsc_the_product_id(); ?>', 'units_grams<?php echo wpsc_the_product_id(); ?>')" 
...
?>
						<form class='product_form'  enctype="multipart/form-data" action="<?php echo str_replace("amp;","",$action); ?>" method="post" name="product_<?php echo wpsc_the_product_id(); ?>" id="product_<?php echo wpsc_the_product_id(); ?>">
		                 	<input type="hidden" name="product_<?php echo wpsc_the_product_id(); ?>_single" id="product_<?php echo wpsc_the_product_id(); ?>_single" value="true" />
						<?php do_action('wpsc_product_addon_after_descr', wpsc_the_product_id()); ?>
<?php /** the variation group HTML and loop */?>
							<div class="wpsc_variation_forms" style="height:73px;">
							<?php $i = 0; ?>
...
<?php /** the variation HTML and loop */?>
								<div class="variation_select">
									<select class='wpsc_select_variation wide' name="variation[<?php echo wpsc_vargrp_id(); ?>]" 
...
<?php endif ; ?>
                        ...
                        <?php endif; ?>	
                    	<?php //if (isset($_GET['key'])) : ?>
                    		<div class="add_prod" <?php if (!isset($_GET['key'])) echo 'style="display:none;"'; ?>><a href="<?php echo get_site_url()."/?page_id=".$_GET['page_id']."&category=".$_GET['category']."&product_id=".$_GET['product_id']; ?>" title="Add new Product">[Add a new product...]</a></div>
                    	<?php //endif; ?>
						<div class="prod_form">
<?php	/**/
		if (isset($_GET['key']) && $wpsc_cart->get_cart_item_by_key($_GET['key'])) { //get_cart_item(wpsc_the_product_id())) {
...
<?php 
	if (isset($_GET['custom']) && $_GET['custom'] == 1)
		echo "<script>document.getElementById('units_grams".wpsc_the_product_id()."').focus()</script>"; ?>