...
function transaction_results($sessionid, $echo_to_screen = true, $transaction_id = null) {
	...
	$product_list = "";
	$report_product_list = "";
	$receipt_list = "Thank you for purchasing with ButcherShop, any items to be shipped will be processed as soon as possible. All prices include tax and postage and packaging where applicable. You ordered the following items:\n\r\n\r";
			...
			//	do_action('wpsc_confirm_checkout', $purchase_log['id']);
				...
				if($link != '' && (!empty($link))) {
				  $additional_content = apply_filters('wpsc_transaction_result_content', array("purchase_id" =>$purchase_log['id'], "cart_item"=>$row, "purchase_log"=>$purchase_log));
					...
					$receipt_list .= ($i+1)." ".$product_data['name'].stripslashes($variation_list)."  x".$row['quantity']."  ".$receipt_price."\n\r";

					$product_list_html_table .= "<tr><td style='text-align:right'>".($i+1)."</td><td>".$name[0]."<br /><span class='greytextsmall'>".$name[1]."</span></td><td>".$row['quantity']."</td><td><span id='checkout_shipping' class='pricedisplay3'>R".$unit_price."</span></td><td><span id='checkout_shipping' class='pricedisplay3'>".$message_price."</span></td></tr>";
					if ($shipping > 0) $product_list_html .= " &nbsp; ". __('Shipping', 'wpsc').":".$shipping_price ."\n\r";
					if ($i==0) 
						$padding = "    ";
					else
						$padding = "";
					$report_product_list.= $padding.($i+1)." ".$product_data['name'].stripslashes($variation_list)."  x".$row['quantity']."  ".$message_price ."\n\r";
				}
			  ...
			  $total_shipping += $purchase_log['base_shipping'];
				...
				$total_shipping_receipt = str_replace(",", "", $total_shipping_email);
				$total_price_email.= __('Total', 'wpsc').": ".trim(substr(nzshpcrt_currency_display($total,1,true),2))."\n\r";
				$total_price_receipt = str_replace(",", "", $total_price_email);
...
/*        $message = str_replace('%product_list%',"stuff",$message);
		...
		$message = $receipt_list_purchase_no.$receipt_list.$total_shipping_receipt.$total_price_receipt;
