...
*/	
	...
	$receipt_list = "<table style='background:#fff;width:500px!important;border-collapse:collapse;border-spacing:0;font-family: Arial, sans-serif;'>
						        	<tr>
   										<td style='height:22px;color:#000;font-size:14px;font-weight:bold;border:1px solid #888;text-align:left;padding:2px;width:330px;'>Product</td>
	...
	if(is_numeric($sessionid)) {
		...
		if(($cart != null) && ($errorcode == 0)) {
			...
			//	do_action('wpsc_confirm_checkout', $purchase_log['id']);
				...
				$total+=($row['price']*ceil($row['quantity']));
				...
				/*if($link != '' && (!empty($link))) {
				  $additional_content = apply_filters('wpsc_transaction_result_content', array("purchase_id" =>$purchase_log['id'], "cart_item"=>$row, "purchase_log"=>$purchase_log));
					...
					$product_list_html_table .= "<tr><td style='text-align:right'>".($i+1)."</td><td>".$name[0]."<br /><span class='greytextsmall'>".str_replace($name[0],"",$row['name'])."</span></td><td>".$row['quantity']."</td><td><span id='checkout_shipping' class='pricedisplay3'>R".str_replace(",", "", $unit_price)."</span></td><td><span id='checkout_shipping' class='pricedisplay3'>".str_replace(",", "", $message_price)."</span></td></tr>";
					$receipt_list .= "<tr><td style='color:#888;font-size:12px;font-weight:bold;line-height:18px;padding:4px;vertical-align:middle;border: 1px solid #AAA;background: white;text-align:left;'>"
													.$name[0]
													."<br /><span style='font-size:11px;'>".str_replace($name[0],"",$row['name'])."</span></td>"
				...
				$trace="<br/><strong>Trace: </strong>".str_replace(" | ",null,$purchase_log["trace"]);
										
				$status="{$trace}<br/><strong><font color='red'>Payment FAILED</font></strong>";
				if ($purchase_log['processed']==2)
					$status="{$trace}<br/><strong><font color='blue'>Payment was SUCCESSFUL</font></strong>";
...
***********************************************/
				...
				$report_user = "<div style='text-transform:uppercase;font-weight:bold'>". __('Customer Details', 'wpsc')."</div>\n\r";
				$report_user .= "<div style='text-decoration:underline;'>Billing Info</div> \n\r";
			...
			$report_user .= "\n\r<div style='text-decoration:underline;'>Shipping Info</div>\n\r";
			...
			$report_user .= "\n\r";
			$message = "Dear ".$name."\n\n\rThank you for purchasing with The Butcher Shop & Grill, any items to be shipped will be processed as soon as possible. All prices include tax and postage and packaging where applicable. \n\r\n\rYou ordered the following items:\n\r\n\r".$message;
				if(($email != '') && ($purchase_log['email_sent'] != 1)) {
...
/*
				...
				$report_user .= "\n\r";
*/
				$report = $report_user.$report.$status;
				...
				if (isset($_SESSION['iveri_processed'])) {
					if ($_SESSION['iveri_processed']=="success") { //If transaction was successful then empty the cart
