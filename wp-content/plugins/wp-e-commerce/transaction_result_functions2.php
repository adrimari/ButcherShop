			...
			//	do_action('wpsc_confirm_checkout', $purchase_log['id']);
				...
				if($link != '' && (!empty($link))) {
				  $additional_content = apply_filters('wpsc_transaction_result_content', array("purchase_id" =>$purchase_log['id'], "cart_item"=>$row, "purchase_log"=>$purchase_log));
					...
					$report_product_list.= $i." ". $product_data['name'] .stripslashes($variation_list)."  x".$row['quantity']."  ".$message_price ."\n\r";
				}
			  ...
			  $total_shipping += $purchase_log['base_shipping'];
				...
				$total_shipping_html.= "<tr><td colspan='4' style='text-align:right'>".__('Total Shipping', 'wpsc').": </td><td><span id='checkout_shipping' class='pricedisplay3'>".str_replace(",", "", substr(nzshpcrt_currency_display($total_shipping,1,true),2))."</span></td></tr>";
				$total_price_html.= "<tr><td colspan='4' style='text-align:right'>".__('Total', 'wpsc').": </td><td><span id='checkout_shipping' class='pricedisplay3'>".str_replace(",", "", substr(nzshpcrt_currency_display($total, 1,true),2))."</span></td></tr>";
				if(isset($_GET['ti'])) {
