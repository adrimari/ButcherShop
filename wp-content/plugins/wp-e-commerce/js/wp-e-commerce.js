...
function testing(id) {
	...
	function checknumber(units, price, id, product_id, minimum, maximum) {
		switch (units) {
			case "grams": clear1(product_id, "units_kilograms"+product_id, "total_kilograms"+product_id, "units_grams"+product_id);
							break;
			case "kilograms": clear1(product_id, "units_grams"+product_id, "total_grams"+product_id, "units_kilograms"+product_id);
							break;
		}
	...
	function enable_custom(id, group_id) {
		...
		e.reset();
		e = document.getElementById('units_grams'+id);
		e.focus();
	}
...
/*			button = 'custom_product_'+id+'_submit_button';
		...
		var i = document.getElementById('total_price_var' + id);
		if (element_total_price) {
			...
			if (e.value == "") {
				...
				//h.value = parseFloat(h.value * e.value).toFixed(2);
			}
		} else {
			//alert(f.innerHTML);
/*			for (var i = 0; i<100; i++) {
			...
			if (e.value == "") {
				...
				i.value = (Math.abs(parseFloat(e.value)) * p).toFixed(2).toString();
			}
	...
	function test(id, grp_id) {
		...
		ajaxRequest.open("GET", "https://shop.thebutchershop.co.za/wp-content/plugins/wp-e-commerce/ajax_example.php" + queryString, true);
...
/*	function clear(item) {
	...
	function rounding(id, units, price, product_id, minimum, maximum) {
		var e = document.getElementById(id);
		var length = e.value.length;
		if (length >= 1)
		{
			if (length == 1) {
				...
				if(e.value % 10 < 5) {
					...
					if (e.value[0] == '9' && e.value[1] == '9')
						e.value = '990';
					else if (e.value[1] == '9') {
							var index0 = parseInt(e.value[0]) + 1;
							var index1 = 0; 
							var index2 = 0; 
							e.value = index0.toString() + index1.toString() + index2.toString();
					} else {
						e.value = e.value[0].toString() + (parseInt(e.value[1]) + 1).toString() + '0';
					}
				}
			}
		}
		//update_qty(id);
		checknumber(units, price, id, product_id, minimum, maximum);
	}
	
	function clear1(id, units, total, referer) {
		var e = document.getElementById(units);
		var f = document.getElementById(total);
		var r = document.getElementById(referer);
		if(r.value != '') {
		//alert(e.value);
			if (e.value != '') {
				e.value = '';
...
//     console.log((button_offset['left'] - container_offset['left'] + 10));
	var parentform = jQuery(parent_form).attr('id') + "_single";
	var single = document.getElementById(parentform);
	if (single.value == "true") {
    	jQuery('#fancy_notification').css("left", (button_offset['left'] - 169) + 'px');
    	jQuery('#fancy_notification').css("top", (button_offset['top'] - 289) + 'px');
	} else {
    	jQuery('#fancy_notification').css("left", (button_offset['left'] - 149) + 'px');
    	jQuery('#fancy_notification').css("top", (button_offset['top'] - 102) + 'px');
	}
