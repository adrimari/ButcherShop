...
function wpsc_admin_variation_forms($variation_id =  null) {
  ...
  if($variation_id > 0 ) {
	...
	$variation_unit = $wpdb->get_var("SELECT `unit` FROM `".WPSC_TABLE_PRODUCT_VARIATIONS."` WHERE `id`='$variation_id' LIMIT 1");
	$variation_association = $wpdb->get_var("SELECT `variation_association` FROM `".WPSC_TABLE_PRODUCT_VARIATIONS."` WHERE `id`='$variation_id' LIMIT 1");
...
/* echo "<script>alert('variation_values: ".$variation_values[0]['name']."')</script>"; */
  ...
  ?>
	<table class='category_forms'>
		<tr>
			<td><input type='radio' name='custom' value='Custom' id='custom' onChange="enable_customvals();" <?php if($variation_values[0]['name'] == 'custom') echo "checked"; ?>/>Allow visitor to enter their own values</td>
			<td>&nbsp;</td>
		</tr>
		<tr>
			<td colspan="2">
				<div id='customvals' style="display:<?php if($variation_values[0]['name'] == 'custom') echo "block"; else echo "none"; ?>">
					<table border="1" cellspacing="0" cellpadding="0">
						<tr>
							<td><?php echo __('Name', 'wpsc');?>:</td>
							<td><input type='text' class="text" name='nameCustom' id='nameCustom' value='<?php echo $variation_name; ?>' /></td>
						</tr>
					</table>
				</div>
			</td>
		<tr>
			<td><input type='radio' name='custom' value='Range' id='custom' onChange="enable_vals();" <?php if($variation_values[0]['name'] != 'custom' || $variation_values[0]['name'] == '') echo "checked"; ?> />Allow visitor to select from a range of values</td>
			<td>&nbsp;</td>
		</tr>
		<tr>
			<td colspan="2">
				<div id='vals' style="display:<?php if($variation_values[0]['name'] != 'custom') echo "block"; else echo "none"; ?>;">
					<table border="1" cellspacing="0" cellpadding="0">
						<tr>
							<td><?php echo __('Name', 'wpsc');?>:</td>
							<td>
                            	<input type='text' class="text" name='nameRange' id='nameRange' value='<?php echo $variation_name; ?>' />
                            	<input type='hidden' class='variation_values_id' name='variation_values_id[]' value='custom' />
                            </td>
						</tr>
                        <tr>
							<td><?php echo __('Unit', 'wpsc');?>:</td>
							<td>
                                <input type='text' class="text" name='unit' id='unit' value='<?php echo $variation_unit; ?>' />
                            </td>
						</tr>
						<tr>
							<td><?php echo __('Variation Values', 'wpsc');?>:</td>
							<td>
								...
								?>
                                   ...
                                   <?php //echo "<script>alert('".$variation_value['name']."')</script>"; ?>
										<?php if($variation_value['name'] == 'custom') { ?>
											<input type='text' class='text' name='variation_values[<?php echo $variation_value['id']; ?>]' value='' />
								...
								?>
                                </div><a href='#' class='add_variation_item_form'>+ <?php _e('Add Value'); ?></a>
							</td>
						</tr>
					</table>
				</div>
