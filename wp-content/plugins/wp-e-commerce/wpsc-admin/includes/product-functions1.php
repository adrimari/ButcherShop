...
//exit('<pre>'.print_r($post_data, true).'</pre><pre>'.print_r($_POST, true).'</pre>');
 ...
 /**
	* wpsc_insert_product function used in admin section 
	*
	* @param unknown 
	* @return unknown
*/
...
function wpsc_insert_product($post_data, $wpsc_error = false) {
  ...
  foreach($product_columns as $column => $default) {
    if(isset($post_data[$column]) || ($post_data[$column] !== null) ) {
			...
			if($post_data[custom_units] == 'on')
				$update_values[custom_units] = 1;
			else
				$update_values[custom_units] = 0;
			if($post_data[units_grams] == 'on')
				$update_values[units_grams] = 1;
			else
				$update_values[units_grams] = 0;			
			if($post_data[units_kilograms] == 'on')
				$update_values[units_kilograms] = 1;
			else
				$update_values[units_kilograms] = 0;
			if($post_data[units_ounces] == 'on')
				$update_values[units_ounces] = 1;
			else
				$update_values[units_ounces] = 0;
			if($post_data[units_pounds] == 'on')
				$update_values[units_pounds] = 1;
			else
				$update_values[units_pounds] = 0;
