...
/*		echo "<script>alert('".$_POST['custom']."')</script>";
		echo "<script>alert('".$_POST['nameRange']."')</script>";
		echo "<script>alert('".$_POST['nameCustom']."')</script>";*/
		if($_POST['custom'] == 'Custom') {
			/*echo "<script>alert('".$_POST['name']."')</script>";*/
			$variation_sql = "INSERT INTO `".WPSC_TABLE_PRODUCT_VARIATIONS."` (`name`, `variation_association`) VALUES ( '".$_POST['nameCustom']."', '".$_POST['unit']."', null);";
			/*echo "<script>alert('".$variation_sql."')</script>";*/
			if($wpdb->query($variation_sql)) {
				...
				//foreach($variation_values as $variation_value) {
					...
					//$num++;
				//}
				$wpdb->query("INSERT INTO `".WPSC_TABLE_VARIATION_VALUES."` ( `name` , `variation_id` )  VALUES ('custom', '".$variation_id."')");
			...
			$variation_sql = "INSERT INTO `".WPSC_TABLE_PRODUCT_VARIATIONS."` (`name`, `unit`,`variation_association`) VALUES ( '".$_POST['nameRange']."', '".$_POST['unit']."', 0);";
			if($wpdb->query($variation_sql)) {
				$variation_id = $wpdb->get_results("SELECT LAST_INSERT_ID() AS `id` FROM `".WPSC_TABLE_PRODUCT_VARIATIONS."` LIMIT 1",ARRAY_A);
				$variation_id = $variation_id[0]['id'];
				$variation_values = $_POST['new_variation_values'];
	...
	if(($_POST['submit_action'] == "edit") && is_numeric($_POST['variation_id'])) {
		...
		if($_POST['new_variation_values'] != null) {
			...
			$wpdb->query("INSERT INTO `".WPSC_TABLE_VARIATION_VALUES."` ( `name` ,  `unit`, `variation_id` )  VALUES ".implode(", ", $variation_value_sql_items));
		}
   
		if($_POST['custom'] == 'Custom')
		{
			$updatesql = "UPDATE `".WPSC_TABLE_PRODUCT_VARIATIONS."` SET `name` = '".$wpdb->escape($_POST['nameCustom'])."', `unit` = '".$wpdb->escape($_POST['unit'])."' WHERE `id`='".$variation_id."' LIMIT 1";
		} else {
			$updatesql = "UPDATE `".WPSC_TABLE_PRODUCT_VARIATIONS."` SET `name` = '".$wpdb->escape($_POST['nameRange'])."', `unit` = '".$wpdb->escape($_POST['unit'])."' WHERE `id`='".$variation_id."' LIMIT 1";
		}
