...
class my_shipping {
	...
	function getQuote() {
		...
		{
			if($my_shipping_rates['saturday'])
			{
				return array ("UK - Special Delivery (next day)" => (float) $my_shipping_rates['special'],
		              "UK - Recorded Delivery" => (float) $my_shipping_rates['recorded'],
					  "UK - 1st Class" => (float) $my_shipping_rates['first'],
					  "UK - Saturday Guaranteed Special Delivery" => (float) $my_shipping_rates['saturday']
					  );
			}
			else
			{
